...
@pytest.mark.integration
@pytest.mark.parametrize(
    "pokemon1, pokemon2, expected_winner",
    [
...
def test_chat_battle_winner(client, pokemon1, pokemon2, expected_winner):
    ...
    json_response = response.json()
    answer = json_response["answer"]
    assert verify_result_with_llm(
        f"expected to win: {expected_winner}", f"actual winner - {answer}"
    )
...
def test_battle_with_nonexistent_pokemons(client, pokemon1, pokemon2):
    ...
    assert (
        ...
        == "Could not analyze the battle due to invalid Pokémon. Please check the spelling of Pokémon names."
    )
